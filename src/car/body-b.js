// CANDIDATE B — the car, exterior, with the shape table rewritten.
//
...
//
// WHAT IT SCORES, `node tools/silhouette.mjs b`, against the shipped car:
//
//                              stock            this
//     side on                  72.6%            81.3%
//         aspect               3.93 (20% off)   3.26   (ref 3.243)
//     rear, no background      93.3%            95.4%
//         aspect               1.38             1.35   (ref 1.349)
//     triangles                10,216           9,464
//     draw calls               3                3
//     ink, tools/inkmeter.mjs  34.1%            34.4%  (target 34-44%)
//
// ---------------------------------------------------------------------------
// READ THIS BEFORE TUNING ANYTHING AGAINST THE SIDE-ON SCORE.
//
// THE SIDE-ON COMPARISON IS BETWEEN MIRROR IMAGES, and 81.3% is very close to
// the most that number can ever be.
//
// tools/silhouette.mjs photographs the side view at az = +PI/2. The camera is
// then on +X, which is the car's RIGHT flank, and it comes out with the nose
// pointing to the RIGHT of frame. ref/side-nobg.png is a drawing of the car's
// LEFT flank: its nose points LEFT. The instrument therefore compares our right
// flank against a drawing of the left one, and a car is not symmetric front to
// back, so the two can never coincide.
//
// This is measurable rather than arguable. Take the reference's own mask, flip
// it, and score it against itself the way the harness scores us:
//
//     reference vs its own mirror image                80.9%
//     this car, mirrored back the right way round      94.7%
//     this car, as the harness grades it               81.3%
//     the shipped car, mirrored back the right way     83.8%
//     the shipped car, as the harness grades it        72.6%
//
// So a PERFECT copy of the drawing would be graded 80.9% by this harness, and
// this car is already above that. The 94.7% is the number that says how close
// the profile is; the 81.3% is what the instrument can see of it.
//
// The consequence for anyone tuning: past this point, MAKING THE CAR MORE LIKE
// THE DRAWING MAKES THE PRINTED SCORE GO DOWN, toward 80.9. One change in this
// file did exactly that and was kept — the ink allowance below took the true
// shape from 93.7% to 94.4% and the printed score from 81.4% to 81.0%. The only
// way to push the printed number much higher is to build the car back to front,
// which would score in the low nineties and drive in reverse.
//
// The fix belongs in the harness (az = -PI/2, or flip the reference mask), not
// in the car, so it is not done here — this file may only touch itself.
//
// THE REAR IS NOT AFFECTED. A car is symmetric left to right, so the rear
// reference scores 99.0% against its own mirror; 95.4% there means 95.4%.
//
...
//
// The four that matter most are 0.355, 0.475, 0.620 and 0.860: the cowl, the
// top of the windscreen, the back of the roof and the base of the backlight.
// Those four are the cabin, and the cabin is what the eye uses to tell a
// Camaro from a Mustang.

const STATION_Z = [
  0.000, 0.020, 0.045, 0.075, 0.110, 0.150, 0.193, 0.240, 0.280, 0.315,
  0.355, 0.395, 0.435, 0.475, 0.520, 0.565, 0.620, 0.660, 0.700, 0.740,
  0.785, 0.825, 0.860, 0.895, 0.925, 0.950, 0.972, 1.000,
].map(uz);
...
const ZONE = [
  ...
  'screen', 'screen', 'screen',                                 // 0.355 .. 0.475
  'cabin', 'cabin', 'cabin',                                    // 0.475 .. 0.620
  'sail', 'sail', 'sail', 'sail',                               // 0.620 .. 0.785
  'deck', 'deck', 'deck', 'deck',                               // 0.785 .. 0.925
...
const CROSS = [
  ...
  { u: 0.475, r0: 6, r1: 10 },                  // top of the windscreen
  { u: 0.620, r0: 6, r1: 10, ink: 'trim' },     // top of the backlight — bright
  { u: 0.660, r0: 2, r1: 6 },                   // door shut, rear
  { u: 0.785, r0: 6, r1: 10 },                  // backlight base / boot shut
...
const GLOW_BOX = { x0: xy(0.46), x1: xy(1.06), y0: vy(0.455), y1: vy(0.610) };
...
function skinOf(zone, span, C, arch, skyward) {
  ...
  switch (span) {
    ...
    case 6:
      ...
      //
      // THE WINDSCREEN COUNTS AS THE CABIN HERE, and on the shipped car it did
      ...
      // is plainly visible from dead side-on. Painted green from the belt up,
      // as it was, the car has a metal panel where its windscreen should be and
      // the cabin reads a third shorter than the drawing's.
      return zone === 'cabin' || zone === 'screen' ? at(C.glass, 1) : C.green;
    case 7: return glass ? lit ?? at(C.glass, 2) : C.green;   // outer top edge
    case 8: return glass ? lit ?? at(C.glass, 2) : C.purple;  // THE STRIPE
    case 9: return glass ? lit ?? at(C.glass, 2) : C.green;   // centre of the top
...
const TAIL_PANEL_LO = vy(0.435), TAIL_PANEL_HI = vy(0.628);
...
function tailFeatures(C) {
  ...
  const lens = (x0, x1) => f.push({
    x0: xy(x0), x1: xy(x1), y0: vy(0.455), y1: vy(0.610),
    spec: at(C.lamp, 2), ink: at(C.chrome, 2), segs: 3, source: true,
  });
